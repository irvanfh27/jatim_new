<template>
    <v-app id="inspire">
        <v-form v-model="valid" v-if="!loading">
            <v-card>
                <v-card-title>
                    <span class="headline" v-if="this.$route.params.uuid">Edit PO</span>
                    <span class="headline" v-else> Create PO</span>
                </v-card-title>
                <v-card-text>
                    <v-container>
                        <v-row>
    ...
    export default {
        ...
        methods: {
            ...
            async insertPODetail() {
                const payload = {
                    ...
                    uom_id: this.formModal.uomId,
                    vendor_id: this.form.general_vendor_id
                };
                try {
                    const res = await axios.post(this.$parent.MakeUrl('po/po-detail'), payload);
                    Vue.swal({
                        icon: "success",
                        title: "Success!",
                        text: "Successfully Insert Data!"
            ...
            getPODetail() {
                axios.get(this.$parent.MakeUrl("po/po-detail?noPO=" + this.form.no_po))
                    .then(res => {
            ...
            deletePODetail(item) {
                Vue.swal({
                    ...
                    if (result.isConfirmed) {
                        axios.post(BaseUrl("po/po-detail") + '/' + item.uuid, {_method: 'DELETE'});
                        Vue.swal(
                            'Deleted!',
        ...
        computed: {
            totalAmount: function () {
                return this.formModal.harga ? this.formModal.qty * this.formModal.harga : 0;
            },
            totalPPN: function () {
